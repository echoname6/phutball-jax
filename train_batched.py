...
def generate_curriculum_batch_fast(
    rng: jax.Array,
    env_config: EnvConfig,
    batch_size: int,
) -> Tuple[np.ndarray, np.ndarray, np.ndarray, dict]:
    """
    Fast vectorized curriculum generation - NO Python loops.

    Generates 1-jump winning states only (vertical jumps, no noise men).
    Fully vectorized using NumPy broadcasting.

    Returns:
        states: (batch_size, 6, rows, cols)
    ...
    # Split RNG and generate random values
    rng, player_rng, col_rng, len_rng = jax.random.split(rng, 4)

    # Random choices for each example (as numpy for speed)
    players = np.array(jax.random.randint(player_rng, (batch_size,), 0, 2)) + 1  # 1 or 2
    ball_cols = np.array(jax.random.randint(col_rng, (batch_size,), 0, cols))
    jump_lens = np.array(jax.random.randint(len_rng, (batch_size,), 1, 5))  # 1-4 men

    # Landing rows depend on player
    landing_rows = np.where(players == 1, rows - 2, 1)

    # Direction: P1 moves down (+1), P2 moves up (-1)
    dr = np.where(players == 1, 1, -1)

    # Ball position
    ball_rows = landing_rows - (jump_lens + 1) * dr
    ball_rows = np.clip(ball_rows, 2, rows - 3)

    # Recalculate jump_len
    jump_lens = np.abs(landing_rows - ball_rows) - 1
    jump_lens = np.maximum(jump_lens, 1)

    # Build all boards at once using broadcasting
    # Base board template: (rows, cols)
    base_board = np.zeros((rows, cols), dtype=np.int32)
    base_board[0, :] = END_HI
    base_board[1, :] = END_HI
    base_board[rows - 2, :] = END_LO
    base_board[rows - 1, :] = END_LO

    # Broadcast to (batch_size, rows, cols)
    boards = np.broadcast_to(base_board, (batch_size, rows, cols)).copy()

    # Place balls using advanced indexing
    batch_idx = np.arange(batch_size)
    boards[batch_idx, ball_rows, ball_cols] = BALL

    # Place men for each example
    # Men are at positions ball_row + k*dr for k in 1..jump_len
    max_jump = 4
    for k in range(1, max_jump + 1):
        man_rows = ball_rows + k * dr
        # Only place if k <= jump_len and position is valid
        valid = (k <= jump_lens) & (man_rows >= 0) & (man_rows < rows)
        valid_batch = batch_idx[valid]
        valid_man_rows = man_rows[valid]
        valid_cols = ball_cols[valid]
        boards[valid_batch, valid_man_rows, valid_cols] = MAN

    # Build network input directly (6 channels)
    # Channel 0: current player's pieces (none for curriculum - ball only)
    # Channel 1: opponent's pieces (none)
    # Channel 2: ball position
    # Channel 3: men positions
    # Channel 4: current player indicator
    # Channel 5: turn progress (0 for curriculum)

    # Actually, need to use state_to_network_input format
    # Simpler: build the 6-channel representation directly

    states = np.zeros((batch_size, 6, rows, cols), dtype=np.float32)

    # Channel 0: Ball position
    states[batch_idx, 0, ball_rows, ball_cols] = 1.0

    # Channel 1: Men positions (from boards where value == MAN)
    states[:, 1, :, :] = (boards == MAN).astype(np.float32)

    # Channel 2: Endzone markers for current player's goal
    # P1's goal is bottom (END_LO), P2's goal is top (END_HI)
    for i in range(batch_size):
        if players[i] == 1:
            states[i, 2, rows-2:rows, :] = 1.0  # P1 wants to reach bottom
        else:
            states[i, 2, 0:2, :] = 1.0  # P2 wants to reach top

    # Channel 3: Opponent's endzone
    for i in range(batch_size):
        if players[i] == 1:
            states[i, 3, 0:2, :] = 1.0
        else:
            states[i, 3, rows-2:rows, :] = 1.0

    # Channel 4: Current player indicator (1 if P1, 0 if P2)
    states[:, 4, :, :] = (players == 1)[:, None, None].astype(np.float32)

    # Channel 5: Turn progress (0 for curriculum states)
    # Already zeros

    # Winning actions
    landing_cols = ball_cols  # Vertical jump, same column
    winning_actions = total_positions + landing_rows * cols + landing_cols

    # Create policy targets (one-hot)
    policies = np.zeros((batch_size, action_space_size), dtype=np.float32)
    policies[batch_idx, winning_actions] = 1.0

    # Values: all +1
    values = np.ones(batch_size, dtype=np.float32)

    stats = {
        'curriculum_1jump': batch_size,
        'curriculum_2jump': 0,
        'curriculum_3jump': 0,
        'curriculum_4jump': 0,
